    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{aulas.length} aula{aulas.length !== 1 ? "s" : ""} registrada{aulas.length !== 1 ? "s" : ""}</p>
        </div>
        ...
        {aulas.map(a => (
          <div key={a.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition-shadow">
            ...
            <div className="flex gap-2 mt-4 pt-3 border-t border-gray-100">
              <button onClick={() => openEdit(a)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100"><Edit2 className="w-3 h-3" /> Editar</button>
              <button onClick={() => setConfirmDelete(a.id)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-red-600 bg-red-50 rounded-lg hover:bg-red-100"><Trash2 className="w-3 h-3" /> Excluir</button>
            </div>
          </div>
        ))}
      </div>
